...
draw:
    #!/usr/bin/env bash
    set -euo pipefail
    keymap -c "{{ draw }}/config.yaml" parse -c 10 -z "{{ config }}/base.keymap" --virtual-layers Combos >"{{ draw }}/base.yaml"
...
test $testpath *FLAGS:
    ...
    diff -auZ ${config_dir}/keycode_events.snapshot ${build_dir}/keycode_events.log   fi
