...
S = lambda key: f"LS({key})"
C = lambda key: f"LC({key})"
A = lambda key: f"LA({key})"
G = lambda key: f"LG({key})"

# Home Row Mod Call Helpers
SL = lambda key: hrm_l(SFT, key)
CL = lambda key: hrm_l(CTL_GUI, key)
AL = lambda key: hrm_l(ALT, key)
ML = lambda key: hrm_l(GUI_CTL, key)
SR = lambda key: hrm_r(SFT, key)
CR = lambda key: hrm_r(CTL_GUI, key)
AR = lambda key: hrm_r(ALT, key)
MR = lambda key: hrm_r(GUI_CTL, key)
...
# Mod Morphs
bsdel = ModMorph("bsdel", "BACKSPACE", "DELETE", mods=[ALT, GUI_CTL, SFT])
...
dot_col = ModMorph("dot_col", "DOT", "COLON", mods=[SFT])
com_sem = ModMorph("com_sem", "COMMA", "SEMICOLON", mods=[SFT])
sqt_dqt = ModMorph("sqt_dqt", "SQT", "GB_DQT", mods=[SFT])
lpar_lt = ModMorph("lpar_lt", "LPAR", "LT", mods=[SFT])
rpar_gt = ModMorph("rpar_gt", "RPAR", "GT", mods=[SFT])
...
if __name__ == "__main__":
  ...
  KeymapGenerator(keyboards=KEYBOARDS).generate_all(output_directory)
