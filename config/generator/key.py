...
class BehaviorCall:
    ...
    def resolve(self, ctx_idx: int = 0, layer_indices: Optional[Dict[str, int]] = None, total_contexts: int = 1) -> Key:
        rendered_args = []
        for a in self.args:
            if isinstance(a, CtxKey):
                ...
                if hasattr(a, "max_contexts") and callable(getattr(a, "max_contexts")) and a.max_contexts() > 1:
