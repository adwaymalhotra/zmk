...
class TestProperty(unittest.TestCase):
  ...
  def test_property_rendering(self):
    ...
    self.assertEqual(
        Prop.str("flavor", "tap-preferred").render().strip(), 'flavor = "tap-preferred";'
    )
    self.assertEqual(
        Prop.bindings("bindings", "&kp A", "&kp B").render().strip(), "bindings = <&kp A>, <&kp B>;"
    )
    self.assertEqual(
        Prop.positions("key-positions", [0, 1, 2]).render().strip(), "key-positions = <0 1 2>;"
    )
    self.assertEqual(
        Prop.mods("mods", ["MOD_LGUI", "LALT"]).render().strip(), "mods = <(MOD_LGUI|MOD_LALT)>;"
    )
    self.assertEqual(
        Prop.mods("mods", ["LCTRL", "LSHFT"]).render().strip(), "mods = <(MOD_LCTL|MOD_LSFT)>;"
    )
    self.assertEqual(Prop.flag("slow-release").render().strip(), "slow-release;")
    self.assertEqual(
        Prop.delete("ignore-modifiers").render().strip(), "/delete-property/ ignore-modifiers;"
    )
